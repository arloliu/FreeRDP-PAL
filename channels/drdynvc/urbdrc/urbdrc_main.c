...
strToHex(const char * cstr)
{
	...
	for(i=strlen(str)-1, j=1; i >= 0; i--, j++)
	{
		if(str[i]>0x60&&str[i]<0x67)
			str[i] -= 0x57;
		else if(str[i]>0x29&&str[i]<0x40)
			str[i] -= 0x30;

		for (temp = str[i], k=1; k<j; k++)
			temp = temp<<4;
...
func_container_id_generate(IUDEVICE * pdev, char * strContainerId)
{
	...
	sprintf(containerId, "\\V%04XP%04XR%04X", idVendor, idProduct, bcdDevice);
...
func_instance_id_generate(IUDEVICE * pdev, char *strInstanceId)
{
	...
	sprintf(instanceId, "USB_%04x%04x%04x", idVendor, idProduct, bcdDevice);
	/* format */
...
urbdrc_search_usb_device(void * arg)
{
	...
	int numobj, timeout;
	int success = 0, error, on_close = 0;
		...
		if (wait_obj_is_set(mon_fd))
		{
			...
			if (dev) {
				...
				if (strcmp(action, "add") == 0)
				{
					...
					idVendor = strToHex(udev_device_get_sysattr_value(dev,
						"idVendor"));
					idProduct = strToHex(udev_device_get_sysattr_value(dev, 
						"idProduct"));
					...
					while(dvc_channel && searchman->has_next(searchman))
					{
							...
							sdev->idProduct == idProduct)
						{
							...
							if (success)
							{
								...
								error = urdbrc_send_virtual_channel_add(
									dvc_channel, 
									0);
...
DVCPluginEntry(IDRDYNVC_ENTRY_POINTS * pEntryPoints)
{
	...
	if (urbdrc == NULL)
	{
		...
		if (data->data[2] && strncmp((char *)data->data[2], "debug", 5) == 0)
			urbdrc_debug = 0;
		else
			urbdrc_debug = 10;
