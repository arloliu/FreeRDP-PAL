...
retry:
	if (libusb_try_lock_events(ctx) == 0) {	
		if (completed == NULL || !*completed) {
			...
			LLOGLN(libusb_debug, ("doing our own event handling\n"));
	...
	if (!libusb_event_handler_active(ctx)) {
		...
		LLOGLN(libusb_debug, ("event handler was active but went away, retrying\n"));
		goto retry;
	}

	LLOGLN(libusb_debug, ("another thread is doing event handling\n"));
...
static IUDEVICE*
udev_init(UDEVICE* pdev, uint16_t bus_number, uint16_t dev_number)
{
...
IUDEVICE *
udev_new_by_id(uint16_t idVendor, uint16_t idProduct)
{
	UDEVICE*        pdev;
	uint16_t        bus_number;
	uint16_t        dev_number;
	...
	LLOGLN(10, ("USB ID: %4X:%4X \n", idVendor, idProduct));
		
	pdev->libusb_handle = libusb_open_device_with_vid_pid (NULL, 
		idVendor, idProduct);    
	if (pdev->libusb_handle == NULL)
	{
		printf("USB renew: Device Not Fined!!\n"); 
		return NULL;
	}
		
	pdev->libusb_dev = libusb_get_device(pdev->libusb_handle);
	if (pdev->libusb_dev == NULL){
		printf("USB renew: Error to call libusb_get_device()!!\n"); 
		return NULL;
	}

	bus_number = libusb_get_bus_number (pdev->libusb_dev);
	dev_number = libusb_get_device_address (pdev->libusb_dev);
